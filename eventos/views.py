...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        draw.text((234, 673), f"{participante.username}", font=fonte_nome, fill=(0, 0, 0))
        draw.text((761, 790), f"{evento.nome}", font=fonte_info, fill=(0, 0, 0))
        draw.text((822, 861), f"{evento.carga_horaria} horas", font=fonte_info, fill=(0, 0, 0))
        output = BytesIO()
